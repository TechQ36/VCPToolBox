...
### 2. BackendImageGen（自选模型生图 · 自有 API）

- 与 KolorsGen 同样的调用方式，但走 **config.env** 中的 API_URL + 图像模型（WhitelistImageModel / VarBackendImageModel），不经过硅基流动。
- 调用示例：
```
...
resolution:「始」1024x1024「末」
```
- 分辨率可选：同上。可选 `seed:「始」整数「末」`。需在全局配置中设置 `WhitelistImageModel` 或 `VarBackendImageModel`（如 `gemini-3-pro-image-[星星公益站-反重力渠道]`），并重启 VCP。
